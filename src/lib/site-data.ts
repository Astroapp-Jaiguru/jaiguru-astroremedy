...
export const getSiteData = cache(async (): Promise<SiteData> => {
  let siteSettings: Awaited<ReturnType<typeof prisma.siteSetting.findMany>> = [];
  let socialRows: Awaited<ReturnType<typeof prisma.socialLink.findMany>> = [];
  let announcementRows: Awaited<ReturnType<typeof prisma.announcement.findMany>> = [];

  try {
    [siteSettings, socialRows, announcementRows] = await Promise.all([
    ...
    // Keep public pages renderable while the optional database is not configured.
  }
