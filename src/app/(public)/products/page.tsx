...
export default async function ProductsPage({ searchParams }: PageProps) {
  ...
  let navNodes: Awaited<ReturnType<typeof fetchNavNodes>> = [];

  try {
    [categories, navNodes] = await Promise.all([
    ...
    // The catalogue renders its empty state until the configured database is available.
  }

  const result = await fetchProductsPage(query, navNodes).catch(() => ({
    products: [],
    total: 0,
    pages: 1,
    current: 1,
