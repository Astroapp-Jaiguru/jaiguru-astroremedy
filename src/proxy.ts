...
export function proxy(request: NextRequest) {
  ...
  const isVendorPublicRoute =
    pathname === "/vendor/register" || pathname === "/vendor/signin";
  const isBuyerPublicRoute = pathname === "/buyer/signin";
  const isSupplierPublicRoute =
    pathname === "/supplier/register" || pathname === "/supplier/signin";
  const isMarketplaceRoute =
    (pathname.startsWith("/vendor") && !isVendorPublicRoute) ||
    (pathname.startsWith("/buyer") && !isBuyerPublicRoute) ||
    (pathname.startsWith("/supplier") && !isSupplierPublicRoute);

  const hasSession = request.cookies.has(SESSION_COOKIE);

  if ((isAdminRoute || isMarketplaceRoute) && !hasSession) {
  ...
  if (pathname.startsWith("/admin") || isMarketplaceRoute) {
    for (const [k, v] of Object.entries(NO_STORE_HEADERS)) res.headers.set(k, v);
  }
  return res;
}
